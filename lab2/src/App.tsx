...
const stringSanitizer = (in_data: string) => {
	return in_data.replace(/[^a-zA-Z]+/g, '');
}
...
const App = () => {
	...
	const onCardNumberChange = (newValue: string) => {
		if (newValue.length >= maxCardNumberLength) return;
	...
	const onCardHolderChange = (newValue: string) => {
		if (newValue.length > 30) return;
		setCardHolder(stringSanitizer(newValue));
	}
					...
					<ImageBackground
						...
						{!backside ? (
							<>
								...
								<Text style={style.cardNumberText}>
									CARD NUMBER
								</Text>
								...
								<Text style={style.cardHolderText}>
									CARD HOLDER
								</Text>
								...
								<Text style={style.expirationDateText}>
									EXPIRATION DATE
								</Text>
